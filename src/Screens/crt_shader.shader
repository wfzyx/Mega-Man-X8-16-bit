...
vec3 fetch_pixel(vec2 uv, vec2 off, sampler2D iChannel0){
	...
	vec3 clr = texture(iChannel0 , pos, -16.0).rgb + noise;
	return clr;
}
...
vec3 Horz3(vec2 pos, float off, sampler2D iChannel0){
	vec3 b = fetch_pixel(pos, vec2(-1.0, off),iChannel0);
	vec3 c = fetch_pixel(pos, vec2( 0.0, off),iChannel0);
	vec3 d = fetch_pixel(pos, vec2( 1.0, off),iChannel0);
...
vec3 Tri(vec2 pos, sampler2D iChannel0){
	vec3 clr = fetch_pixel(pos, vec2(0.0),iChannel0);
