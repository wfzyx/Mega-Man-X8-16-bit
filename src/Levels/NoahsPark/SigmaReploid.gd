...
func step_foward():
	...
	if abs(node_destination) < step_distance:
		step_distance = abs(node_destination)
	...
	node_destination = node_destination - step_distance * facing_direction
	Tools.timer(.3, "play_r", step)
	if abs(node_destination) > 0.0:
		tween.add_wait(.5)
	...
	print("Destination " + str(node_destination))
...
func moving_towards_back() -> bool:
	if node_destination > 0 and facing_direction == -1:
		return true
	if node_destination < 0 and facing_direction == 1:
		return true
	return false
